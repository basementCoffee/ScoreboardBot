...
module.exports = {
  ...
  execute(message, args, Discord, bot, data) {
    const newEmbed = new Discord.MessageEmbed()
      .setColor("#76ee00")
      .setTitle(
        "Click the below link to view entry and screenshots for verification"
      )
      .setDescription(
      ...
      .addFields({
        name: "Guide:",
        value:
          "React with a X or check and the spreadsheet will update accordingly",
      })
    ...
    bot.channels.cache
      ...
      .then((a) => {
        ...
        const filter = (reaction, user) => {
          return (
            user.id !== bot.user.id &&
            ["613845951055921158", "613845951341395977"].includes(
              reaction.emoji.id
            )
          );
        };
        ...
        collector.on("collect", (reaction, reactionCollector) => {
          ...
          if (reaction.emoji.id === "613845951055921158") {
            ...
            message.channel.send(
              "Added to sheet. #leaderboard-scores is updated."
            );
            // bot-testing channel id: 802071947088625694;    embed id: 841537152910491659; leaderboard channel id: 841438933824569375
          } else {
            message.channel.send(
              "Sorry your high-score entry has been rejected by an officer. Make sure you include your screenshots and the right category."
            );
          }
        });
      });
